...
def modlines(request):
    ...
    return (
        ...
        + MODLINES[1:]
        if EXPORTS_KEY in request.fixturenames
        else MODLINES
    )


def test_export_sensitivity(tmpdir, target_package, parse_style, exports, modlines):
    ...
    assert [] == missing, "Missing {} doc target(s):\n{}".format(
        len(missing), "\n".join(missing)
    )


def test_export_specificifty(tmpdir, target_package, parse_style, exports, modlines):
    ...
    assert [] == present, "{} unexpected doc target(s):\n{}".format(
        len(present), "\n".join(present)
    )
...
def try_cat_module(folder):
    ...
    if len(mods) == 1:
        with open(mods[0], "r") as f:
            print(
                "Contents of module {} from folder {}:\n{}".format(
                    mods[0], folder, f.read()
                )
            )
    else:
        print(
            "Not printing module contents; did not find exactly 1 module in "
